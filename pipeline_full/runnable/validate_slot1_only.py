"""Validate `generated_slot1_prediction.csv` against `../../frozen/slot1_prediction.csv`.

Pass criteria:
  - Schema matches: columns == ['trustii_id', 'risk_hepatic_event', 'risk_death']
  - Row count == 423
  - No NaNs
  - trustii_id sets equal AND aligned in order after sort by trustii_id
...
If rank-identity FAILS, stop and report — do NOT modify the script, the
cached intermediates, or the output to force a match. The frozen file is
authoritative.
"""
...
REGEN = HERE / "generated_slot1_prediction.csv"
...
def main() -> None:
    ...
    if regen.isna().any().any():
        fail(f"NaNs in regenerated: {regen.isna().sum().to_dict()}")
    if frozen.isna().any().any():
        fail(f"NaNs in frozen: {frozen.isna().sum().to_dict()}")
    ...
    for col in ("risk_hepatic_event", "risk_death"):
        ...
        if not np.array_equal(rr, rf):
            ...
            print(f"FAIL: RANK MISMATCH in {col}: {n_diff} positions differ", file=sys.stderr)
            diffs = np.where(rr != rf)[0][:10]
            for i in diffs:
                tid = regen_sorted["trustii_id"].iat[int(i)]
                print(f"  trustii_id={tid}: regen rank={rr[i]}, frozen rank={rf[i]}", file=sys.stderr)
            rank_pass = False
        else:
