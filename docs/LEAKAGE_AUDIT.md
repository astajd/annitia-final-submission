...
Because the public leaderboard is now closed, no regenerated alternative file can be scored to confirm equivalence to the submitted file. This is why the submitted file is treated as the frozen, authoritative artifact and the reproduction claim is made relative to it. The authoritative verification path (`build_slot1_only.py`) regenerates the submitted file rank-identically from cached intermediates without raw data; see `REPRODUCIBILITY.md`.
